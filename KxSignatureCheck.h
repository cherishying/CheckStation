#ifndef _KXSIGNATURECHECKH
#define _KXSIGNATURECHECKH

//add by lyl 2017/3/16
//签名条检测模块
//初始参数版本名为：SignatureCheck1.0
...
#pragma pack(push, 1)
	...
	struct  Parameter
	{
		Parameter()
		{
			memcpy(m_szVersion, "SignatureCheck1.0", _VersionNameLen);
			m_nIsCheckSignature = 0;
		...
		//部分参数对签名条来说无用参数，开始设计有问题，为了兼容只好改成这样
		char              m_szNoUse[_VersionNameLen];  //多余，为保证一致
