...
#include "KxSignatureCheck.h"
#include "KxReadXml.h"

CKxSignatureCheck::CKxSignatureCheck()
{
...
bool CKxSignatureCheck::ReadXml(const char* filePath, const kxCImageBuf& BaseImg)
{
	m_nUseXml = 1;
	//检测控制参数
...
bool CKxSignatureCheck::ReadParaXml(const char* filePath)
{
	//重要参数
	std::string szResult;
...
bool CKxSignatureCheck::ReadXmlinEnglish(const char* filePath, const kxCImageBuf& BaseImg)
{
	m_nUseXml = 1;
	//检测控制参数
...
bool CKxSignatureCheck::ReadParaXmlinEnglish(const char* filePath)
{
	//重要参数
	std::string szResult;
	...
	nSearchStatus = KxXmlFun::SearchXmlGetValue(filePath, "Signature", "SearchValue", szResult);
	if (nSearchStatus)
	{
		int nStatus = KxXmlFun::FromStringToInt(szResult, m_nSearchEdgeThresh);
		if (!nStatus)
...
bool CKxSignatureCheck::ReadVesion1Para( FILE* fp)    //读取版本1参数
{
	if (fread(&m_hParameter.m_nIsCheckSignature, sizeof(int), 1, fp) != 1)
		return false;
	if (fread(&m_hParameter.m_Point, sizeof(kxPoint<int>), _Max_Point, fp) != _Max_Point)
		return false;
	if (fread(&m_hParameter.m_nPointCount, sizeof(int), 1, fp) != 1)
		return false;
	if (fread(m_hParameter.m_szNoUse, sizeof(m_hParameter.m_szNoUse), 1, fp) != 1)
		return false;
...
bool CKxSignatureCheck::ReadVesion2Para(FILE* fp)    //读取版本2参数
{
	if (fread(&m_hParameter.m_nIsCheckSignature, sizeof(int), 1, fp) != 1)
		return false;
	if (fread(&m_hParameter.m_Point, sizeof(kxPoint<int>), _Max_Point, fp) != _Max_Point)
	...
	if (fread(&m_hParameter.m_nGrayLowOffset, sizeof(int), 1, fp) != 1)
	{
		return false;
	}
	return true;
...
bool CKxSignatureCheck::ReadVesion3Para(FILE* fp)    //读取版本3参数
{
	if (fread(&m_hParameter.m_nIsCheckSignature, sizeof(int), 1, fp) != 1)
		return false;
	if (fread(&m_hParameter.m_Point, sizeof(kxPoint<int>), _Max_Point, fp) != _Max_Point)
	...
	if (strcmp(m_hParameter.m_szNoUse, "CheckControl4.0") == 0)
	{
...
bool CKxSignatureCheck::ReadVesion4Para(FILE* fp)    //读取版本3参数
{
	if (fread(&m_hParameter.m_nIsCheckSignature, sizeof(int), 1, fp) != 1)
		return false;
	if (fread(&m_hParameter.m_Point, sizeof(kxPoint<int>), _Max_Point, fp) != _Max_Point)
	...
	if (fread(&m_hParameter.m_nPointCount, sizeof(int), 1, fp) != 1)
		return false;
	//新增加参数
	if (fread(&m_hParameter.m_nIsOpenWhiteArea, sizeof(int), 1, fp) != 1)
		return false;
	if (fread(&m_hParameter.m_rcWhiteArea, sizeof(kxRect<int>), 1, fp) != 1)
		return false;
	if (fread(&m_hParameter.m_nWhiteAreaCheckThresh, sizeof(int), 1, fp) != 1)
		return false;
	if (fread(&m_hParameter.m_nSearchEdgeThresh, sizeof(int), 1, fp) != 1)
		return false;
...
//从网络中获取参数
int CKxSignatureCheck::ReadParaFromNet(unsigned char*& point)
{
	...
	if (!m_hImgMask.ReadParaFromNet(point))
		return false;
	if (!m_hPolygonMask.ReadParaFromNet(point))
		return false;
...
int CKxSignatureCheck::LoadTemplateImg(const char* lpszFile)
{
	FILE*   fp;
...
int CKxSignatureCheck::SaveTemplateImg(const char* lpszFile)
{
	FILE*   fp;
...
int CKxSignatureCheck::SaveLearnTemplateImg(const char* lpszFile)
{
	FILE*   fp;
...
int CKxSignatureCheck::WriteLocalLearnImg(FILE* fp)
{
...
int CKxSignatureCheck::LoadLearnTemplateImg(const char* lpszFile)
{
	FILE*   fp;
...
int CKxSignatureCheck::ReadLocalLearnImg(FILE* fp)
{
...
int CKxSignatureCheck::ReadImg(FILE* fp)
{
	if (fread(&m_nModelNumber, sizeof(int), 1, fp) != 1)
		return FALSE;
...
int CKxSignatureCheck::WriteImg(FILE* fp)
{
	if (fwrite(&m_nModelNumber, sizeof(int), 1, fp) != 1)
		return FALSE;
...
bool CKxSignatureCheck::ReadPara(FILE*  fp)
{
	if(fread(m_hParameter.m_szVersion, sizeof(m_hParameter.m_szVersion), 1, fp) != 1)
		return false;
	if(strcmp(m_hParameter.m_szVersion, "SignatureCheck1.0") == 0)   //根据版本标识来判断需要调用的函数
		return ReadVesion1Para(fp);
	else if (strcmp(m_hParameter.m_szVersion, "SignatureCheck2.0") == 0)
		return ReadVesion2Para(fp);
	else if (strcmp(m_hParameter.m_szVersion, "SignatureCheck3.0") == 0)
		return ReadVesion3Para(fp);
	else if (strcmp(m_hParameter.m_szVersion, "SignatureCheck4.0") == 0)
		return ReadVesion4Para(fp);
	else
		return false;
}

bool CKxSignatureCheck::Read( FILE*  fp)
{
	m_nUseXml = 0;
	//检测控制参数
	if(!ReadPara(fp))
	...
	if(m_hParameter.m_nIsCheckSignature)
	{
		//m_nIsCheckH = (int)(m_hParameter.m_nCheckMethod != _Gray_Low_Method);
		//if(m_hParameter.m_nPointCount < 4 || m_hParameter.m_nPointCount > 5)
		//	kxPrintf(KX_ERR, "签名条标定核个数错误");
		for(int i = 0; i < m_hParameter.m_nPointCount; i++)
		{
			m_Offset[i].x = m_hParameter.m_Point[i].x - m_hParameter.m_Point[0].x;
			m_Offset[i].y = m_hParameter.m_Point[i].y - m_hParameter.m_Point[0].y;
		}
...
bool CKxSignatureCheck::WriteVesion1Para( FILE* fp)    //写入版本1参数
{
	if (fwrite(m_hParameter.m_szVersion, sizeof(m_hParameter.m_szVersion), 1, fp) != 1)
		return false;
	if (fwrite(&m_hParameter.m_nIsCheckSignature, sizeof(int), 1, fp) != 1)
		return false;
	if (fwrite(&m_hParameter.m_Point, sizeof(kxPoint<int>), _Max_Point, fp) != _Max_Point)
		return false;
	if (fwrite(&m_hParameter.m_nPointCount, sizeof(int), 1, fp) != 1)
		return false;
...
bool CKxSignatureCheck::WritePara(FILE*  fp)
{
	if(strcmp(m_hParameter.m_szVersion, "SignatureCheck1.0") == 0)   //根据版本标识来判断需要调用的函数
...
bool CKxSignatureCheck::Write(FILE*  fp)
{
	if(!WritePara(fp) )
		return false;
...
int  CKxSignatureCheck::Matting(const kxCImageBuf& SrcImg, bool bModel)
{
	KxCallStatus hCall;
...
int  CKxSignatureCheck::Matting(const kxCImageBuf& SrcImg, bool bModel, KxCallStatus& hCall)
{
	hCall.Clear();
	...
	//梯度
	{
		...
		if (bModel)
		{
			//m_rcBaseSignaturePos = m_rcSignaturePos;
			m_nSignatureModelWidth = pos.Width();
			m_nSignatureModelHeight = pos.Height();
		}
		else
		{
			m_nSignatureWidth = pos.Width();
			m_nSignatureHeight = pos.Height();
		}
		...
		kxRect<int> rc;
		rc.left = (std::max)(nPos[0][0] - dx, 0);
		rc.right = (std::min)(SrcImg.nWidth - 1, nPos[0][1] + dx);
		rc.top = (std::max)(nPos[1][0] - dy, 0);
		rc.bottom = (std::min)(SrcImg.nHeight - 1, nPos[1][1] + dy);
		...
		if (m_hParameter.m_nIsOpenWhiteArea)
		{
			m_rcWhiteArea.left = (std::max)(m_rcWhiteArea.left - 15, 0);
			m_rcWhiteArea.top = (std::max)(m_rcWhiteArea.top -15, 0);
			m_rcWhiteArea.right = (std::min)(m_rcWhiteArea.right + 15, m_ImgEdgeTemp.nWidth - 1);
			m_rcWhiteArea.bottom = (std::min)(m_rcWhiteArea.bottom + 15, m_ImgEdgeTemp.nHeight - 1);
			...
			//	m_ImgEdgeTemp.buf + m_rcWhiteArea.left * m_ImgEdgeTemp.nChannel + m_rcWhiteArea.top * m_ImgEdgeTemp.nPitch, m_EdgeImg.nPitch, roiSize);

			IppiSize roiSize = { m_rcWhiteArea.Width(), m_rcWhiteArea.Height() };
			ippiSet_8u_C1R(0,
				m_ImgEdgeTemp.buf + m_rcWhiteArea.left * m_ImgEdgeTemp.nChannel + m_rcWhiteArea.top * m_ImgEdgeTemp.nPitch, m_ImgEdgeTemp.nPitch, roiSize);
		}
		...
		//mask边缘一个像素由于梯度引起的
		roiSize.width = SrcImg.nWidth - 4;
		roiSize.height = SrcImg.nHeight - 4;
		ippiCopy_8u_C1R(m_ImgEdgeTemp.buf + 2 * m_ImgEdgeTemp.nPitch + 2, m_ImgEdgeTemp.nPitch,
			m_EdgeImg.buf + 2 * m_EdgeImg.nPitch + 2, m_EdgeImg.nPitch, roiSize);
...
//学习图像
int  CKxSignatureCheck::LearnTemplate(KxCallStatus& hCall)
{
	hCall.Clear();
	if (!m_hParameter.m_nIsLearn)
	{
...
//残差分析并且合并为一张残差图
int CKxSignatureCheck::ResidualsAnalys(KxCallStatus& hCall)
{
	hCall.Clear();
	for (int i = 0; i < _Check_Layer; i++)
	{
	...
	//加边缘检查
	nOffset[_Check_Layer] = m_nSignatureBorderCheckThresh;
	...
	for (int i = 0; i < _Check_Layer; i++)
	{
		if (m_nAlgorithmStatus[i])
		{
			m_hFun[i].KxMaxEvery(m_ImgCompose[i], m_ImageCc);
		}
	}
	...
	//去线，误报
	Ipp8u pOpenMask[2] = { 1, 1 };
	m_ImgCc.Init(m_ImageCc.nWidth, m_ImageCc.nHeight);
	m_hBaseFun.KxOpenImage(m_ImageCc, m_ImgCc, 1, 2, pOpenMask, hCallInfo1);
	m_hBaseFun.KxOpenImage(m_ImgCc, m_ImgTemp, 2, 1, pOpenMask, hCallInfo1);
...
int CKxSignatureCheck::ConnectAnalysis(const kxCImageBuf& SrcImg, KxCallStatus& hCall)
{
...
int CKxSignatureCheck::SingleImageLearn(const kxCImageBuf& ImgCheck)
{
	KxCallStatus hCall;
...
//单张学习
int CKxSignatureCheck::SingleImageLearn(const kxCImageBuf& ImgCheck, KxCallStatus& hCall)
{
	hCall.Clear();
	KxCallStatus hCallInfo;
...
int CKxSignatureCheck::SingleDefectLearn(const kxCImageBuf& ImgCheck, int nBlobIndex)
{
	KxCallStatus hCall;
...
//单个缺陷学习
int CKxSignatureCheck::SingleDefectLearn(const kxCImageBuf& ImgCheck, int nBlobIndex, KxCallStatus& hCall)
{
	hCall.Clear();
	KxCallStatus hCallInfo;
...
int CKxSignatureCheck::SetMask(const kxCImageBuf& SrcDstImg, const kxCImageBuf& MaskImg, Ipp8u lowGate, Ipp8u highGate)
{
...
int CKxSignatureCheck::AddResidualImage(const kxCImageBuf& CcImg, const kxCImageBuf& SrcImg, int nColorMode, kxCImageBuf& DstImg)
{
...
int CKxSignatureCheck::ShowResidualsImage(const kxCImageBuf& SrcImg, kxCImageBuf& DstImg, int nOffset[5])
{
	KxCallStatus hCall;
...
int CKxSignatureCheck::ShowResidualsImage(const kxCImageBuf& SrcImg, kxCImageBuf& DstImg, int nOffset[5], KxCallStatus& hCall)
{
	KxCallStatus hCallInfo;
	hCall.Clear();
...
int CKxSignatureCheck::AnalysisCheckStatus()
{
	for (int i = 0; i < _Max_Layer; i++)
	{
...
int CKxSignatureCheck::Check(const kxCImageBuf& ImgCheck)
{
	KxCallStatus hCall;
...
int CKxSignatureCheck::Check(const kxCImageBuf& ImgCheck, KxCallStatus& hCall)
{
	return Check(ImgCheck.buf, ImgCheck.nWidth, ImgCheck.nHeight, ImgCheck.nPitch, ImgCheck.nChannel, hCall);
}

int CKxSignatureCheck::Check(const unsigned char* buf, int nWidth, int nHeight, int nPitch, int nChannel, KxCallStatus& hCall)
{
	KxCallStatus hTmpCall;
	hTmpCall.Clear();
	hCall.Clear();
...
int CKxSignatureCheck::SetCheckExceptionStatus(const kxCImageBuf& ImgCheck)
{
...
int CKxSignatureCheck::SaveMidImage(bool bSave)
{
#if defined( _WIN32 ) || defined ( _WIN64 )
	if (bSave)
	{
